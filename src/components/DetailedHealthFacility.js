...
class DetailedHealthFacility extends Component {
  ...
  _onChange = (hf) => {
    if (
      !!hf &&
      (hf.location.id !== (!!this.state.district && this.state.district.id) || hf.level !== this.state.level)
    ) {
      this.setState(
        {
  ...
  render() {
    ...
    let grid = split ? 12 : 6;
    let region =
      (!!value && !!value.location && value.location.parent) || (this.state.district && this.state.district.parent);
...
export default withModulesManager(
  connect(mapStateToProps, mapDispatchToProps)(withTheme(withStyles(styles)(DetailedHealthFacility))),
);
